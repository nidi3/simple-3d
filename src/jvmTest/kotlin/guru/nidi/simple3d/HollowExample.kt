...
fun main() {
    fun base(r: Double, b: Double, h: Double) = csg {
        ...
        val e = r - b
        base + corner.rotateZ(180.deg).translate(e, e, h) +
    ...
    fun top(r: Double, b: Double) = csg {
        ...
        val outerBorders = inner.rotateZ(90.deg).translate(d2, 0, b2) +
                ...
                inner.rotateZ(90.deg).translate(-d3, 0, b2)
        cone + innerBorders + outerBorders
    }
...
//        val dino = dino(b)
//
//        val factor = min(1.5 * r / dino.width, 1.5 * r / dino.height)
//        val scaleDino = dino.csg.scale(factor, factor, 1)
//
//        val base = cube(center = origin, length = v(2 * r, 2 * r, b))
//        val dBase = base - scaleDino.translate(-dino.width * factor / 2, -dino.height * factor / 2, b / 2 - .45)
//        add(
//            dBase,
//            dBase.rotateZ(180.deg).rotateX(90.deg).translate(0, d, d),
//            dBase.rotateZ(180.deg).rotateX(90.deg).translate(0, -d, d),
//            dBase.rotateZ(-90.deg).rotateY(90.deg).translate(d, 0, d),
//            dBase.rotateZ(-90.deg).rotateY(90.deg).translate(-d, 0, d)
//        )

        add(
            base(r, b, h),
            side(r, b, h).translate(0, 2.5 * r, 0),
            side(r, b, h).translate(2.5 * r, 2.5 * r, 0),
            side(r, b, h).translate(0, 5.0 * r, 0),
            side(r, b, h).translate(2.5 * r, 5.0 * r, 0),
            top(r, b).translate(2.5 * r, 0, 0)
        )

//        val b2 = 1.0
//        val d2 = r - 3 * b2 / 2
//        transformed(translate(2.5 * r, 0, 0)) {
//            val inner = cube(center = origin, length = v(2 * (r - b2), b2, 2 * b2))
